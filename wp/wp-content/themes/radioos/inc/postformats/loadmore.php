...
$paged++;
        ...
        if(is_tax()){$cur_cat = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) );}
        wp_reset_query();
        
        if(is_page_template('_projects.php')){
            $_SESSION['category'] = $allcat;
        }elseif(is_tax()){
            $_SESSION['category'] = $cur_cat->term_id;
        }
        $cat_array = explode(',', $_SESSION['category']);
        $count_args=array('tax_query' => array(array('taxonomy' => 'ct_projects','field' => 'term_id', 'terms' => $cat_array)),  'post_type' => 'projects',  'post_status' => 'publish', 'ignore_sticky_posts'=> 1,'posts_per_page'=>-1);
        $post_count = new WP_Query($count_args);
        $remaining = ($post_count->post_count-(get_option('posts_per_page')*($paged-1)));
        if($remaining >= 0){
        ?>                

        <div class="home-portfolio-one load-more-content left load-more-content-<?php echo $number ?>" <?php if(is_home()){echo 'style="margin-top:0"';}?>>
            <div class="bg-load-more left">
                <a class="add-posts-<?php echo $number ?>"></a>
            </div>
    ...
    jQuery(document).ready(function($){
        ...
        $('.add-posts-<?php if($remaining > 0){echo $number;}else{echo '0';} ?>').live('click', function(){
            ...
            jQuery.ajax({
                type: "POST",
                url: "index.php",
                data: {tk_paged:<?php echo $paged?>, cat:'<?php echo $_SESSION['category']?>', all:'<?php if(is_page_template('_projects.php')){echo '1';}elseif(is_tax()){echo '0';}?>'}
            }).done(function( msg ) {
                $newItems = $(msg);
                $('#fakeholder').html('');
