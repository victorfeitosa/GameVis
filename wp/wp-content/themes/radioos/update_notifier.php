...
function update_notifier_menu() {
    global $wp_version;
	$xml = get_latest_theme_version(259200); // This tells the function to cache the remote call for 21600 seconds (6 hours)
	if ( version_compare( $wp_version, '3.4', '>=' ) ) {$theme_data = wp_get_theme(TEMPLATEPATH . '/style.css');}else{$theme_data = get_theme_data(TEMPLATEPATH . '/style.css');} // Get theme data from style.css (current version is what we want)
...
function update_notifier() {
	...
	<div class="wrap">
        ...
        <div id="instructions" style="max-width: 800px;">
            ...
            <p>To update the Theme, login to your account, head over to your <strong><a href="http://www.themeskingdom.com/download-themes/">downloads</a></strong> section and re-download the theme.</p>
	    ...
	    <h3 class="title">Changelog</h3><br />
...
function get_latest_theme_version($interval) {
	...
	$notifier_file_url = 'http://www.themeskingdom.com/demo/index.php?xml='.strtolower($theme_data['Name']);
	$db_cache_field = 'contempo-notifier-cache';
	$db_cache_field_last_updated = 'contempo-notifier-last-updated2';
