...
class tk_Walker_Comment extends Walker_Comment {
    ...
    function start_lvl(&$output, $depth = 0, $args = array()) {
        ...
        <ul <?php comment_class('media-list children comment-child-'.$depth.' comment-' . get_comment_ID()); ?>>
        <?php
    ...
    function start_el(&$output, $comment, $depth = 0, $title='', $args = array(), $id = 0) {
        ...
        <?php endif; ?>
        <li class="comment" id="comment-<?php comment_ID(); ?>" <?php comment_class('media row-fluid comment-' . get_comment_ID()); ?>>
            <article>
                <header class="clearfix">
                    <?php echo get_avatar($comment, $size = '46'); ?>
                    <h5 class="comment-heading"><?php echo get_comment_author_link(); ?></h5>
                    <div class="meta-data">
                        <time datetime="<?php echo comment_date('c'); ?>"><a href="<?php echo htmlspecialchars(get_comment_link($comment->comment_ID)); ?>"><?php printf(__('%1$s', 'tkingdom'), get_comment_date(),  get_comment_time()); ?></a></time>
                        <span>&bull;</span>
                        <?php comment_reply_link(array_merge($args, array('depth' => $depth, 'max_depth' => 4))); ?>
                        <?php edit_comment_link(__('<span>&bull;</span> Edit', 'tkingdom'), '', ''); ?>
                    </div>
                </header>
    ...
    /************************************************************/
    if (have_comments()) : ?>
            <h4 class="comments-title"><span><?php comments_number( '0', '1', '%' ); ?></span> <?php _e('Comments','tkingdom')?></h4>
        ...
        <div class="comment-respond">
            ...
            <?php else : ?>
                <div class="comment-form">
                    <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform"  class="comment-form" onSubmit="return checkForm(this);">
                        <fieldset>
                        ...
                        <?php else : ?>
                            <h2 class="comment-reply-title"><?php _e('Leave a comment', 'tkingdom'); ?></h2>
                        ...
                        <div class="control-group">
                            <div class="controls">
                                <button type="submit" onclick="this.form.submit();" data-hover="<?php _e('POST COMMENT', 'tkingdom')?>" class="btn btn-sm" value="<?php _e('Submit Comment', 'tkingdom'); ?>"><span class="secondary-color-btn"><?php _e('POST COMMENT', 'tkingdom')?></span></button>
                            </div>
                        </div>
