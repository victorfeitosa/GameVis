...
if(!empty($_POST['link'])) {
   ...
   $wpdb->query(
   $wpdb->prepare("INSERT INTO ".$wpdb->prefix."advertising (title, link, imagelink) VALUES ('%s', '%s', '%s' )", $title, $link, $imagelink));
...
if(isset($_GET['delete_advert'])) {
   ...
   $wpdb->query(
	 $wpdb->prepare("DELETE FROM " .$wpdb->prefix. "advertising WHERE ID = '%d'", $advert_id));
}
...
if(isset($_POST['title_edit'])) {    
  ...
  $wpdb->query(
	$wpdb->prepare("UPDATE " .$wpdb->prefix. "advertising SET title= '%s', link = '%s', imagelink= '%s' WHERE ID = '%d'", $title_edit, $link_edit, $imagelink_edit, $post_id));
