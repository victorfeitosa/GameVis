...
jQuery(document).ready(function($){
	...
	function block_size(width) {
		...
		if (width > 0 && width < 130){ span = "col-sm-2"; }
		else if (width == 166){ span = "col-sm-3"; }
		else if (width == 228){ span = "col-sm-4"; }
		else if (width == 290){ span = "col-sm-5"; }
		else if (width == 352){ span = "col-sm-6"; }
		else if (width == 414){ span = "col-sm-7"; }
		else if (width == 476){ span = "col-sm-8"; }
		else if (width == 538){ span = "col-sm-9"; }
		else if (width == 600){ span = "col-sm-10"; }
		else if (width == 662){ span = "col-sm-11"; }
		else if (width == 724){ span = "col-sm-12"; }
	...
	$('ul.blocks li.block').each(function() {
		...
		var element_resize = $('#' + blockID);

		//blocks resizing
		element_resize.resizable(resizable_args);
		...
		//trigger resize
		element_resize.trigger("resize");
		element_resize.trigger("resizestop");
	...
	$( "ul.blocks" ).bind( "sortstop", function(event, ui) {
		...
		if (ui.item.hasClass('ui-draggable')) {
		    ...
		    //set random block id
		    block_number = makeid();
	        jQuery(this).append('<input type="hidden" class="get_id" value="'+block_number+'" />');

	        var blockID = $(this).parents('li').attr('id');
	        //alert(blockID);

	        //resizable_dynamic_width(block_number);

	        setTimeout(function(){
	            jQuery('.get_id').each(function(){
	                var getID = jQuery(this).val();

                    jQuery("#aq_block_"+getID+"_post_types").change(function() {
                     var getPostType =  jQuery(this).find("option:selected").val();

                     if(getPostType == 'ct_team' ) {
                         jQuery('.no-show-aq_block_'+getID+', .column-number-aq_block_'+getID).css('display', 'none');
                         jQuery('.color-shower-aq_block_'+getID).css('display', 'block');
                         jQuery('.description-team-aq_block_'+getID).css('display', 'block');
                     } else if (getPostType == 'ct_gallery' ) {
                         jQuery('.color-shower-aq_block_'+getID).css('display', 'block');
                         jQuery('.no-show-aq_block_'+getID+', .aq_block_'+getID+'-noshow, .text-color-'+getID).css('display', 'none');
                         jQuery('.description-gallery-aq_block_'+getID+', .post-headline-color-aq_block_'+getID+', .column-number-aq_block_'+getID).css('display', 'block');
                     } else if (getPostType == 'ct_client' ) {
                         jQuery('.color-shower-aq_block_'+getID+', .description-client-aq_block_'+getID).css('display', 'block');
                         jQuery('.no-show-aq_block_'+getID+', .column-number-aq_block_'+getID+', .post-headline-color-aq_block_'+getID+', .text-color-aq_block_'+getID).css('display', 'none');
                         jQuery('.description-client-aq_block_'+getID).css('display', 'block');
                     } else if (getPostType == 'post' ) {
                         jQuery('.color-shower-aq_block_'+getID).css('display', 'block');
                         jQuery('.column-number-aq_block_'+getID+', .no-show-aq_block_'+getID).css('display', 'none');
                         jQuery('.'+getID+'-noshow').css('display', 'block');
                     }
                   });
	            });
		    }, 100);
		...
		//Resize element to column width if dragged into column
		var parentBlock = ui.item.parent().parent().parent();
		if(parentBlock.hasClass('block-aq_column_block')) {
			var id_block = ui.item.attr('id');
			var block_drag = $('#' + id_block);
			var parentBlockSize = parentBlock.css('width');
			var elementBlockSize = block_drag.css('width');
			var old_size_class = block_size(elementBlockSize);
			var new_size_class = block_size(parentBlockSize);

			block_drag.removeClass(old_size_class).addClass(new_size_class + ' in-column');
			block_drag.find('.size').val(new_size_class);

			block_drag.resizable('destroy');
		}
	...
	$('#save_template_header, #save_template_footer').click(function() {
		var template_name = $('#tk_pagbuilder_select').val().trim();
