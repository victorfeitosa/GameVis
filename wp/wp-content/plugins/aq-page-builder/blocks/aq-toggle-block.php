<?php
/* Aqua Tabs Block */
if(!class_exists('AQ_Toggle_Block')) {
	class AQ_Toggle_Block extends AQ_Block {
		...
		function __construct() {
			...
			//create the widget
			parent::__construct('AQ_Toggle_Block', $block_options);
		...
		function block($instance) {
			...
			$output = '';
				...
				$output .= '<div id="aq_block_toggles_wrapper_'.rand(1,100).'" class="aq_block_toggles_wrapper margin-bottom-builder">';

				foreach( $tabs as $tab ){
					$output  .= '<div class="aq_block_toggle" style="border:'.$border_color.';background:transparent;">';
						$output .= '<h5 class="tab-head" style="background:rgba('.$bg_color_rgb[0].','.$bg_color_rgb[1].','.$bg_color_rgb[2].','.$tab_opacity.');color:'.$heading_color.';border-radius:0px;">'. do_shortcode(htmlspecialchars_decode($tab['title'])) .'</h5>';
						$output .= '<i class="icon-plus"></i><i class="icon-minus"></i>';
						$output .= '<div class="tab-body close cf" style="background:rgba('.$bg_color_rgb[0].','.$bg_color_rgb[1].','.$bg_color_rgb[2].','.$tab_opacity.');color:'.$text_color.';border-radius:0px;">';
