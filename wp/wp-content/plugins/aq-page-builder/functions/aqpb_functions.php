	...
	********************************************************/
	...
	function aq_field_upload($field_id, $block_id, $media, $media_type = 'image') {
		...
		return $output;
                	...
                	/* Post type select */
	function aq_posttype_select($field_id, $block_id, $post_option, $get_results) {
                  
                    //gets all post types
                    $output = '<select id="'. $block_id .'_'.$field_id.'" name="aq_blocks['.$block_id.']['.$field_id.']">';                        
             
                    $output .= '<option '.selected( $get_results, 'post', false ).' value="post">Post</option>';

                    foreach($post_option as $single_post_type){
                        $post_type_info = get_post_type_object($single_post_type);                        
                        if(!empty($post_type_info->taxonomies)){ 
                            $output .= '<option  '.selected( $get_results, $post_type_info->taxonomies[0], false ).' value="'.$post_type_info->taxonomies[0].'">'.ucfirst($single_post_type).'</option>';
                        }
                    }
                    $output .= '</select>';
                   ...
                   /* Post type select */
	function aq_category_select($field_id, $block_id, $taxonomy_name, $get_results) {

                   $get_tax_terms = get_terms($taxonomy_name);
                               
                    //gets all post types
                    $output = '<select id="'. $block_id .'_'.$field_id.'" name="aq_blocks['.$block_id.']['.$field_id.']">';                    
                    $output .= '<option value="">All</option>';

                    foreach($get_tax_terms as $single_tax){                        
                        if(!empty($single_tax->name)){
                            $output .= '<option '.selected( $get_results, $single_tax->name, false ).'  value="'.$single_tax->name.'">'.$single_tax->name.'</option>';
                        }                                                
                    }
                    $output .= '</select>';
