...
// Testimonial Block
function aq_block_testimonial($block) {
	extract( $block, EXTR_OVERWRITE );
	...
	<p class="description">
		<label for="<?php echo $block_id ?>_name">
			Name
			<input type="text" class="input-full" id="<?php echo $block_id ?>_name" value="User Name" name="<?php echo $block_saving_id ?>[name]">
		</label>
	</p>	
	<p class="description">
		<label for="<?php echo $block_id ?>_text">
...
//Advertisement block
function aq_block_advertisement($block) {
	extract( $block, EXTR_OVERWRITE );
	$block_id = 'aq_block_' . $number;
	$block_saving_id = 'aq_blocks[aq_block_'.$number.']';
