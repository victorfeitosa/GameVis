...
// this was replaced $template_name = isset($_REQUEST['template-name']) && !empty($_REQUEST['template-name']) ? htmlspecialchars($_REQUEST['template-name']) : 'No Title';

if (!empty($_REQUEST['tk_pagebuilder_select'])){
    $template_name = isset($_REQUEST['tk_pagebuilder_select']) && !empty($_REQUEST['tk_pagebuilder_select']) ? htmlspecialchars(get_the_title($_REQUEST['tk_pagebuilder_select'])) : 'No Title';
    $tk_template_id = isset($_REQUEST['tk_pagebuilder_select']) && !empty($_REQUEST['tk_pagebuilder_select']) ? htmlspecialchars($_REQUEST['tk_pagebuilder_select']) : 'No Title';
} elseif (!empty($_REQUEST['template-name'])) {
	...
	case 'create' :
		...
		if(!is_wp_error($new_id)) {
			$selected_template_id = $new_id;
                                                       add_post_meta($new_id, 'tk_page_id', $tk_template_id);
					...
					<div class="aqpb-tabs-wrapper">
						<div class="aqpb-tabs">
							...
							foreach ( (array) $templates as $template ) {
                                                                                                                                    ...
                                                                                                                                    $tk_template_id = get_post_meta($template->ID, 'tk_page_id', true);
                                                                                                                                    
                                                                                                                                    if(!empty($tk_template_id)){
                                                                                                                                        $tk_template_title = get_the_title($tk_template_id);
                                                                                                                                    }
					...
					<form id="update-page-template" action="<?php echo $this->args['page_url'] ?>" method="post" enctype="multipart/form-data">
						<div id="aqpb-header">
								...
								<div id="submitpost" class="submitbox">
									<div class="major-publishing-actions cf" style="display: inline-block">
										...
										<div class="publishing-action">
											<?php submit_button( empty( $selected_template_id ) ? __( 'Create Template', 'framework' ) : __( 'Save Template', 'framework' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_header' ) ); ?>
                                                                                                                                                                                    ...
                                                                                                                                                                                    <?php
                                                                                                                                                                                    /*
                                                                                                                                                                                     * OPTIION ADDED TO DEFAULT AQ BUILDER
                                                                                                                                                                                     * TO ADD PAGE SELECT DEPENDING ON WHAT USER DOES
                                                                                                                                                                                     * EDIT: Danko Dubajic, 02-07-2013
                                                                                                                                                                                     */

                                                                                                                                                                                    if(isset($_GET['template'])){
                                                                                                                                                                                        if(isset($_GET['action']) && gettype($template) != 'object'){
                                                                                                                                                                                            $args = array('name'=>'tk_pagebuilder_select', 'show_option_none'=> 'Select Page');
                                                                                                                                                                                            wp_dropdown_pages($args);
                                                                                                                                                                                        }else{
                                                                                                                                                                                            if( !isset($template) || $_GET['template'] == 0){
                                                                                                                                                                                                if(is_array($template) && $template != 0){
                                                                                                                                                                                                    $selected_page = get_post_meta($template->ID, 'tk_page_id', true);
                                                                                                                                                                                                }
                                                                                                                                                                                                    $args = array('name'=>'tk_pagebuilder_select', 'show_option_none'=> 'Select Page');
                                                                                                                                                                                                    wp_dropdown_pages($args);
                                                                                                                                                                                            } // if template variable is not set and get template is 0
                                                                                                                                                                                        } // check if action is delete and template variable is not object
                                                                                                                                                                                    }else{
