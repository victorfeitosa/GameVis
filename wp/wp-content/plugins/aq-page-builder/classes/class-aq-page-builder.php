...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function init() {
			...
			if(!is_admin()) add_action('wp_enqueue_scripts', array(&$this, 'view_enqueue'), 20); // danko added to 20 to be loaded after bootstrap
		...
		function admin_enqueue() {
            ...
            wp_register_script('bootstrap', $this->url . 'assets/bootstrap/bootstrap.js', array('jquery'), time(), true);
		...
		function view_enqueue() {
			...
			// Themes Kingdom CSS	
			wp_register_style( 'aqpb-tk', $this->url.'assets/css/aqpb-tk.css', array(), time(), 'all');
			wp_enqueue_style('aqpb-tk');

			// front-end js
			wp_register_script('aqpb-view-js', $this->url . 'assets/js/aqpb-view.js', array('jquery'), time(), true);
			wp_enqueue_script('aqpb-view-js');
			
          	wp_register_style('bootstrap', $this->url . 'assets/bootstrap/bootstrap.css');
          	wp_enqueue_style('bootstrap');

         	wp_register_style('tk_aqpb_bootstrap', $this->url . 'assets/bootstrap/tk_aqpb_bootstrap.css');
          	wp_enqueue_style('tk_aqpb_bootstrap');
          	...
          	wp_enqueue_script('flexslider', $this->url . '/assets/flexslider/jquery.flexslider.js', false, false, true );

          	wp_enqueue_script('sly_slider', $this->url . '/assets/sly_slider/sly.slider.js', false, false, true );
		...
		**/
		function update_template($template_id, $blocks, $title) {
			...
			foreach ($blocks as $new_instance) {
                ...
                //edit Danko Dubajic
                $tk_template_id = get_post_meta($template_id, 'tk_page_id', true);
                update_post_meta($tk_template_id, 'tk_page_builder_id', '[template id="'.$template_id.'"]');
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				//template wrapper
				echo '<div id="aq-template-wrapper-'.$template_id.'" class="aq-template-wrapper aq_row row">';
