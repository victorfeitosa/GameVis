...
define(function (require) {
	...
	DotaDriver.prototype.buildData = function (src) {
		...
		var radiant = new data.Team({
			name: 'Radiant',
			rank: 0,
			nation: 'radiant'
		});
		var dire = new data.Team({
			name: 'Dire',
			rank: 0,
			nation: 'dire'
		});
		for (var i in jsonmatch.players) {
			...
			for (var j in jsonheroes.heroes) {
				if (jsonmatch.players[i].hero_id === jsonheroes.heroes[j].id) {
					var iconuri = 'http://cdn.dota2.com/apps/dota2/images/heroes/' +
						(jsonheroes.heroes[i].name.replace('npc_dota_hero_', '')) +
						'_full.png';
					p.addResource('Hero', new data.Resource({
						id: jsonheroes.heroes[j].id,
						name: jsonheroes.heroes[i].localized_name,
						value: 'hero',
						iconuri: iconuri
					}), -1);
				}
			...
			for (var j = 0; j < 6; j++) {
				...
				if (itid !== 0) {
					...
					p.addResource('Item', new data.Resource({
						id: jsonmatch.players[i][item],
						name: jsonitems[itid].localized_name,
						value: 'item',
						iconuri: iconuri
					}), j);
				} else {
			...
			for (var j in jsonmatch.players[i].ability_upgrades) {
				...
				if (ability !== null) {
					var iconuri =
						...
						.name + '_hp1.png';
					var resource = new data.Resource({id: parseInt(ability.id), name: ability.name,
						value: 'skill', iconuri: iconuri});
