...
define(function (require) {
	...
	StatPolygon.prototype.append = function () {
		...
		self.Polygon = self.Group.append('g').classed('stat-polygon-polygroup',
			true);
	...
	function TimeAxis(obj) // an axis with ticks
		{
			...
			if (obj) {
				...
				if (this.Canvas.ClassType === 'Canvas')
					this.Canvas = obj.canvas.getCanvas;
				this.Scale = obj.scale || d3.scale.linear().domain([0, 100]).range([0,
					320]);
	...
	TimeAxis.prototype.append = function () {
		...
		//creates axis here
		self.Axis = d3.svg.axis().scale(self.Scale).orient(self.Orient).tickSize(0); //Assign axis to just created scaled axis
