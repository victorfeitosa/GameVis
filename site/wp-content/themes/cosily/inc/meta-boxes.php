...
$meta_boxes = array(
    ...
    array(
        ...
        'pages' => array('post', 'gallery', 'rooms'), // multiple post types
        'context' => 'normal',
        'priority' => 'high',
        'fields' => array(
    ...
    array(
        ...
        'pages' => array('post', 'page', 'rooms'), // multiple post types
        'context' => 'side',
        'priority' => 'high',
        'fields' => array(
            array(
    ...
    array(
        'id' => 'post_meta11',
        'title' => __('Room Options', tk_theme_name()),
        'pages' => array('rooms'), // multiple post types
        'context' => 'side',
        'priority' => 'high',
        'fields' => array(
            
            array(
                'name' => __('Price per night', tk_theme_name()),
                'desc' => '',
                'id' => $prefix . 'room_price_adult',
                'type' => 'text',
                'std' => ''
            ),
            
            array(
                'name' => __('Number of beds', tk_theme_name()),
                'desc' => '',
                'id' => $prefix . 'number_beds',
                'type' => 'text',
                'std' => ''
            ),
            
            array(
                'name' => __('Room Description', tk_theme_name()),
                'desc' => 'Set room description',
            ...
            array(
                'name' => __('Room Policy', tk_theme_name()),
                'desc' => 'Set room policy',
                'id' => $prefix . 'room_policy',
                'type' => 'plaintextarea',
                'std' => '',
                'options' => array(
                    'rows' => '3',
            ...
            array(
                'name' => __('External Link Reservation', tk_theme_name()),
                'desc' => 'If reservations are turned off you still can set link to external reservations website',
            ...
            array(
                'name' => __('External Link Text', tk_theme_name()),
                'desc' => 'External link text',
                'id' => $prefix . 'external_text',
                'type' => 'text',
                'std' => ''
            ),
    ...
    array(
        'id' => 'post_meta12',
        'title' => __('Heading Colors', tk_theme_name()),
        'pages' => array('page'), // multiple post types
        'context' => 'normal',
        'priority' => 'high',
        'fields' => array(
        ...
        foreach ($this->_meta_box['fields'] as $field) {
            ...
            switch ($field['type']) {
                ...
                case 'text':
                    ...
                    if($field['id'] == 'tk_number_beds') { echo '<p style="color:red;">Required Field</p>'; }
                    
                    if($field['id'] == 'tk_room_price_adult') { echo '<p style="color:red;">Numbers Only</p>'; }
                    break;
                ...
                case 'sidebar':
                    ...
                    <script type="text/javascript">
                        jQuery(document).ready(function(){
                            ...
                            if(current_template == 'page-templates/_gallery_3_columns.php'){
                                jQuery('#post_meta8').attr('style', 'display:none');
                                jQuery('#post_meta13').attr('style', 'display:none');
                            }else if(current_template == 'page-templates/_gallery_3_columns.php'){
                                 jQuery('#post_meta8').attr('style', 'display:none');
                                 jQuery('#post_meta13').attr('style', 'display:none');
                            }else if(current_template == 'page-templates/_reservations.php'){
                                 jQuery('#post_meta8').attr('style', 'display:none');
                                 jQuery('#post_meta13').attr('style', 'display:block');
                            }else if(current_template == 'page-templates/front-page.php'){
                                 jQuery('#post_meta7').attr('style', 'display:none');
                                 jQuery('#post_meta8').attr('style', 'display:none');
                                 jQuery('#post_meta12').attr('style', 'display:none');
                                 jQuery('#post_meta13').attr('style', 'display:block');
                            }else{
                            ...
                            jQuery("#page_template").change(function() {
                                ...
                                if(page_template == 'page-templates/_gallery_4_columns.php'){
                                    jQuery('#post_meta8').attr('style', 'display:none');
                                    jQuery('#post_meta13').attr('style', 'display:none');
                                }else if(page_template == 'page-templates/_gallery_3_columns.php'){
                                     jQuery('#post_meta8').attr('style', 'display:none');
                                     jQuery('#post_meta13').attr('style', 'display:none');
                                }else if(page_template == 'page-templates/_reservations.php'){
                                     jQuery('#post_meta8').attr('style', 'display:none');
                                     jQuery('#post_meta13').attr('style', 'display:block');
                                }else if(page_template == 'page-templates/front-page.php'){
                                     jQuery('#post_meta7').attr('style', 'display:none');
                                     jQuery('#post_meta8').attr('style', 'display:none');
                                     jQuery('#post_meta12').attr('style', 'display:none');
                                     jQuery('#post_meta13').attr('style', 'display:block');
                ...
                case 'colorpicker':                   
                    ...
                    global $wpdb;                        
                        $i = 0;
                        $list_room_options = $wpdb->get_results( "SELECT * FROM ".$wpdb->prefix.tk_theme_name."_room_options");           
                        ...
                        foreach($list_room_options as $room_options){
                            ...
                            foreach($meta as $meta_one) {
                                if($meta_one == $room_options->room_options) {
                                    $checked = 'checked="checked"';
                                } 
                            }
                            
                            echo '<input type="checkbox" value="'.$room_options->room_options.'" name="', $field['id'].'[]', '" id="', $field['id'], '"', $checked, ' /> '.$room_options->room_options.' ('.$room_options->price.')<br />';                            
                            $i++;
                        }
