        ...
        foreach ($_POST as $var => $value) {
            if ($var != '_wpnonce' && $var != '_wp_http_referer' && $var != 'Submit' && $var != 'ilc-settings-submit' && !preg_match("/_required/i", $var)) {//$_POST variables which we don't want to save as options                    
                if ($_GET['tab'] == 'room_options') {
                    $check_result = $wpdb->get_results("SELECT * FROM `" . $wpdb->prefix . "rooms` WHERE room_options = '$value';");
...
function tk_builder_posts($post_id, $build_block_fields) {
    ...
    $tk_gallery_id = get_option('id_gallery_page');
    $tk_gallery4_id = get_option('id_gallery4_page');
    $tk_room_id = get_option('id_rooms_page');
    ...
    foreach ($posts as $post) {   
        ...
        if($post->ID == $tk_blog_id || $post->ID == $tk_contact_id || $post->ID == $tk_testimonials_id || $post->ID == $tk_gallery_id ||$post->ID == $tk_gallery4_id || $post->ID == $tk_room_id){
        } else {
        if ($val == $post->ID) {
            $selected = 'selected';
        } else {
...
function tk_settings_page($par) {
    ...
    <p class="submit" style="clear: both;">
        ...
        <?php if($tab !== 'room_reservations' && $tab !== 'new_reservation'){ ?>
