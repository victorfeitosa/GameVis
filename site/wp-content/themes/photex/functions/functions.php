    ...
    function tk_pageing($custom_query){
        ...
        //if(is_array($pages)) {
            ...
            foreach($pages as $page) {
                ...
                if(strpos($page, 'class="prev')){
                    $page = str_replace('class="prev', 'class="previous', $page);
                }
                if(strpos($page, 'span')){
                ...
                if(strpos($page, 'current')){ 
                    ...
                    $li_class = 'class="pagination-dots"'; 
                    $page = str_replace('…', '...', $page);
                }
                else{ $li_class = ""; }

                echo '<li '.$li_class.'>'.$page.'</li>';
            }
            echo '</ul>';
        //}
    } // pageing function
...
function tk_video_player($url) {
    if (!empty($url)) {
        ...
        ?>
            <figure class="video scalable-wrapper">
...
add_action("after_setup_theme", "tk_create_tables"); //theme switch action
...
add_action('wp_loaded', 'tk_check_for_banner_redirection', 0); //Awaiting for banner click redirections
add_action('wp_loaded', 'tk_check_for_banner_stats', 0);
...
/************************************************************/
    ...
    /*************************************************************/
    function tk_related_posts($post_id, $post_type, $taxonomy){
                ...
                $args = array(
                    ...
                    'tax_query'      => array( //(array) - use taxonomy parameters (available with Version 3.1).
                                            ...
                                            array(
                                                ...
                                                'terms'    => $cats, //(int/string/array) - Taxonomy term(s).
    ...
    /*************************************************************/
    /*******   FUNCTION ADD JS INFINITE SCROLL *******************/
    /*************************************************************/
    function tk_infinite_scroll(){
    ...
    /*************************************************************/
    /*********** CHANGE IMAGE SIZE FOR WOOCOMMERCE ***************/
    /*************************************************************/
    global $pagenow;
    ...
    /*************************************************************/
    /*********** FUNCTION GET EXCERPT FOR HOME SLIDER ************/
    /*************************************************************/
    function tk_get_home_title(){
        ...
        if(isset($_POST['post_id'])){
            ...
            if(strlen($post_title) > 128){
                $post_title = substr($post_title, 0, 128).'...';
            }

            $post_title = '<a class="active" href="'.$permalink.'">'.$post_title.'</a>';

            $vote_count = get_post_meta($post_id, "votes_count", true); $liked = ""; if ( tk_has_voted($post_id) ) { $liked = 'liked'; }
            if($vote_count == '') { $vote_count = 0; } else { $vote_count = $vote_count; }

            $post_like = '<a href="#" class="verticalize '.$liked.' active" data-post_id="'.$post_id.'" >
                <i class="icon-heart"></i>
                <i class="icon-heart-full"></i>
                <span class="like-counter">'.$vote_count.'</span>
            </a>';

            $result = array($post_title, $post_like);
    ...
    add_action('wp_ajax_nopriv_get_home_title', 'tk_get_home_title');
    add_action('wp_ajax_get_home_title', 'tk_get_home_title');
    ...
    /*************************************************************/
    /*********** FUNCTION SEND EMAIL CONTACT POPUP ***************/
    /*************************************************************/
    function tk_send_mail_popup(){
        $nonce   = $_POST['nonce'];
        ...
        $body = __("You received e-mail from ", 'tkingdom').$name."  [".$email."] ". __(" using ", 'tkingdom').$sitename."\n\n\n".$message;

        $send = wp_mail($to, $subject, $body, $headers);
        if($send) { _e('Message sent!'); } else { _e('Message not sent!'); }

        die();
    }
    add_action('wp_ajax_nopriv_send_mail_popup', 'tk_send_mail_popup');
    add_action('wp_ajax_send_mail_popup', 'tk_send_mail_popup');

    /*************************************************************/
    /*** Remove Shortcodes Bootstrap ***/
    /*************************************************************/
    add_action('wp_enqueue_scripts', 'tk_remove_shortcodes_bootstrap', 100);
    function tk_remove_shortcodes_bootstrap()  {
