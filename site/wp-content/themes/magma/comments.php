...
class tk_Walker_Comment extends Walker_Comment {
    ...
    function start_el(&$output, $comment, $depth = 0, $title='', $args = array(), $id = 0) {
        ...
        <?php endif; ?>
        <li id="comment-<?php comment_ID(); ?>" <?php comment_class('media row comment-' . get_comment_ID()); ?>>
            <article>            
                <header>
                    <?php echo get_avatar($comment, $size = '46'); ?>
                    <h5 class="comment-heading"><?php echo get_comment_author_link(); ?></h5>
                    <div class="meta-data">
                        <time datetime="<?php echo comment_date('c'); ?>"><div class="comment-date"><?php printf(__('%1$s', 'tkingdom'), get_comment_date(),  get_comment_time()); ?></div></time>
                        <span>&bull;</span>
    ...
    if (have_comments()) : ?>
        <section id="comments">
            <h2 class="comments-title"><?php _e('Comments','tkingdom')?> <span><?php comments_number( '0', '1', '%' ); ?></span></h2>
