    ...
    class twitter_bootstrap_nav_walker extends Walker_Nav_Menu {
        ...
        function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0) {
            ...
            if (strcasecmp($item->title, 'divider')) {
                ...
                if (($item->type == 'taxonomy') && ($menu_location == 'secondary')) {
                    $tax_args = array(
                                'posts_per_page' => -1,
                                'post_type' => 'post',
                                'post_status' => 'publish',
                    ...
                    $post_count = count(get_posts($tax_args));

                    $item_output .= ($args->has_children) ? ' <strong>'.$post_count.'</strong></a>' : '<strong>'.$post_count.'</strong></a>';
...
function tk_sidebar_field($tag) {
    ...
    ?>
    <tr class="form-field">
        <th scope="row" valign="top"><label for="sidebar-selection"><?php _e('Select Sidebar', 'tkingdom'); ?></label></th>
        <td>
...
?>
            ...
            <br />
            <span class="description"  style=" margin-bottom: 25px;display: inline-block;"><?php _e('Select sidebar for this category.', 'tkingdom'); ?></span>
        </td>
    </tr>
    <?php
}
...
function tk_category_field($tag) {
    if (isset($_GET['tag_ID'])) {
        ...
        $cat_layout = get_option("category_layout_$tag_id");
    }
    ?>
    <tr>
        <th scope="row" valign="top"><label for="category_color"><?php _e('Category Display', 'tkingdom'); ?></label></th>
        <td>
            <select name="category_display" id="category_display">
                <option value="cat_layout_one" <?php
               if (!isset($cat_display) || $cat_display == 'cat_layout_one') {
                   echo 'selected';
               }
    ?>><?php _e('One Column', 'tkingdom') ?></option>
                <option value="cat_layout_two" <?php
            if (isset($cat_display) && $cat_display == 'cat_layout_two') {
                echo 'selected';
            }
    ?>><?php _e('Two Columns', 'tkingdom') ?></option>
            </select>
            <br />
            <span class="description"  style=" margin-bottom: 25px;display: inline-block;"><?php _e('Select how to display this category.', 'tkingdom'); ?></span>
        </td>
    </tr>
    <tr>
        <th scope="row" valign="top"><label for="category_color"><?php _e('Sidebar Position', 'tkingdom'); ?></label></th>
        <td>
            <select name="category_layout" id="category_layout">
                <option value="sidebar-right" <?php
               if (!isset($cat_layout) || $cat_layout == 'sidebar-right') {
                   echo 'selected';
               }
    ?>><?php _e('Sidebar Right', 'tkingdom') ?></option>
                <option value="sidebar-left" <?php
            if (isset($cat_layout) && $cat_layout == 'sidebar-left') {
                echo 'selected';
            }
    ?>><?php _e('Sidebar Left', 'tkingdom') ?></option>
                <option value="fullwidth" <?php
            if (isset($cat_layout) && $cat_layout == 'fullwidth') {
                echo 'selected';
            }
    ?>><?php _e('Fullwidth', 'tkingdom') ?></option>    
            </select>
            <br />
            <span class="description"  style=" margin-bottom: 25px;display: inline-block;"><?php _e('Select how to display this category.', 'tkingdom'); ?></span>
        </td>
...
function tk_save_category_field($term_id) {
        $tag_id = $term_id;
        if (!isset($_POST["category_display"]) && empty($_POST["category_display"])) {
            $_POST["category_display"] = 'cat_layout_one';
        }
        if (!isset($_POST["category_layout"]) && empty($_POST["category_layout"])) {
            $_POST["category_layout"] = 'sidebar-right';
        }

        //save the option array
        update_option("category_display_$term_id", $_POST["category_display"]);
        update_option("category_layout_$term_id", $_POST["category_layout"]);
}
...
function dimox_breadcrumbs() {
    ...
    if ( !is_home() && !is_front_page() || is_paged() ) {
        ...
        if ( get_query_var('paged') ) {
            ...
            echo $delimiter .$before . __('Page', 'tkingdom') . ' ' . get_query_var('paged'). $after;
            if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
        }
    ...
    <?php if ($rating_status == 'yes') { ?>
                jQuery('.stars-rater .<?php echo $class ?>').rating({
                    callback: function(value, link){
                        ...
                        jQuery('.single-rating-one .single-user-rate').html('').html('<?php _e('User Rating: ', 'tkingdom'); ?> '+new_total+' ('+old_rate+'<?php _e(' votes)', 'tkingdom'); ?>' );
                    }
                });
    <?php } else { ?>
                jQuery('.stars-rater .<?php echo $class ?>').rating({
...
/* * ********************************************************** */
/* * **********FUNCTION FOR RETRIEVING RATED POSTS ************ */
/* * ********************************************************** */

function tk_get_all_rated_posts($limit){
    $args = array(
        'post_status' => 'publish', 
        'posts_per_page' => $limit, 
        'meta_key' => 'tk_enable_rating',
        'meta_value' => 'on',
        'orderby' => 'modified'
    );

    $rated_posts_query = get_posts($args);
    return $rated_posts_query;
}
...
<th><label for="twitter">Twitter</label></th>
    <td><input type="text" name="twitter" id="twitter" value="<?php echo esc_attr(get_the_author_meta('twitter', $user->ID)); ?>" class="regular-text" /><br />
        <span class="description">Please enter your Twitter username (e.g. themeskingdom)</span>
    </td>
</tr>    
<th><label for="facebook">Facebook</label></th>    
    <td><input type="text" name="facebook" id="facebook" value="<?php echo esc_attr(get_the_author_meta('facebook', $user->ID)); ?>" class="regular-text" /><br />
        <span class="description">Please enter your Facebook username (e.g. themeskingdom)</span>
    </td>
</tr>
<tr>
<th><label for="googleplus">Google+</label></th>    
    <td><input type="text" name="googleplus" id="googleplus" value="<?php echo esc_attr(get_the_author_meta('googleplus', $user->ID)); ?>" class="regular-text" /><br />
        <span class="description">Please enter your Google+ number (e.g. 12345678912345)</span>
    </td>
</tr>    
<tr>
<th><label for="dribbble">Dribbble</label></th>    
    <td><input type="text" name="dribbble" id="dribbble" value="<?php echo esc_attr(get_the_author_meta('dribbble', $user->ID)); ?>" class="regular-text" /><br />
        <span class="description">Please enter your Dribbble username (e.g. themeskingdom)</span>
    </td>
</tr>    
<tr>
<th><label for="pinterest">Pinterest</label></th>    
    <td><input type="text" name="pinterest" id="pinterest" value="<?php echo esc_attr(get_the_author_meta('pinterest', $user->ID)); ?>" class="regular-text" /><br />
        <span class="description">Please enter your Pinterest username (e.g. themeskingdom)</span>
    </td>
</tr>    
<tr>
<th><label for="behance">Behance</label></th>    
    <td><input type="text" name="behance" id="behance" value="<?php echo esc_attr(get_the_author_meta('behance', $user->ID)); ?>" class="regular-text" /><br />
        <span class="description">Please enter your Behance username (e.g. themeskingdom)</span>
    </td>
</tr>    
<tr>
<th><label for="youtube">YouTube</label></th>    
    <td><input type="text" name="youtube" id="youtube" value="<?php echo esc_attr(get_the_author_meta('youtube', $user->ID)); ?>" class="regular-text" /><br />
        <span class="description">Please enter your YouTube username (e.g. themeskingdom)</span>
    </td>
</tr> 
<tr>   
<th><label for="instagram">Instagram</label></th>    
    <td><input type="text" name="instagram" id="instagram" value="<?php echo esc_attr(get_the_author_meta('instagram', $user->ID)); ?>" class="regular-text" /><br />
        <span class="description">Please enter your Instagram username (e.g. themeskingdom)</span>
    </td>
</tr>    
<tr>
<th><label for="rssfeed">RSS Feed</label></th>    
    <td><input type="text" name="rssfeed" id="rssfeed" value="<?php echo esc_attr(get_the_author_meta('rssfeed', $user->ID)); ?>" class="regular-text" /><br />
        <span class="description">Please enter your RSS feed url (e.g. http://www.themeskingdom.com/feed)</span>
    </td>
</tr>  
<tr>  
<th><label for="vimeo">Vimeo</label></th>    
    <td><input type="text" name="vimeo" id="vimeo" value="<?php echo esc_attr(get_the_author_meta('vimeo', $user->ID)); ?>" class="regular-text" /><br />
        <span class="description">Please enter your Vimeo username (e.g. themeskingdom)</span>
    </td>
</tr>
</table>
<?php } 
...
add_filter('wp_list_categories', 'add_span_cat_count');
    ...
    $links = str_replace('</a> (', '</a> <span class="cat-number">', $links);
...
/*************************************************************/
/*******   HOOK MODERNIZR TO HEAD  **********/
/*************************************************************/

function hook_modernizr(){
    $output="<script src='".get_template_directory_uri()."/script/modernizr/modernizr.js'></script>";
    echo $output;
}

add_action('wp_head','hook_modernizr');
